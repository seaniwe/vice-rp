...
mp.events.add('render', () => {
    ...
    if (controls.isControlJustPressed(0, controlsIds.F5)) {
        // mp.events.callRemote('flyAdminAcess');
        mp.events.call('flyAdminAcessAccept');
    } else if (fly.flying) {
        let updated = false;
        var position = {x:0,y:0,z:0};
        if(mp.players.local.vehicle){
            // mp.console.logInfo("Аыфв", true, true);
            position = mp.players.local.vehicle.position;
        }else{
...
mp.events.add('flyAdminAcessAccept',()=>{
    const controls = mp.game.controls;
    const fly = global.fly;
    direction = global.gameplayCam.getDirection();
    coords = global.gameplayCam.getCoord();
